...
import { AUDIO_TRACK_CONSTRAINTS, SCREEN_SHARING_MEDIA_CONSTRAINTS, VIDEO_TRACK_CONSTRAINTS } from "./constraints";


// https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia
const getCameraOrMicrophone = async (videoDeviceId: string, audioDeviceId: string): Promise<MediaStream> => {
...
const getScreenshareExample = async (deviceId: string): Promise<MediaStream> => {
...
const attachVideo = (stream: MediaStream) => {
  ...
  const videoPlayer = document.querySelector<HTMLVideoElement>("video-player-id")
...
document.querySelector<HTMLVideoElement>("#grant-permission-btn")?.addEventListener("click", async () => {
...
document.querySelector<HTMLVideoElement>("#enumerate-devices-btn")?.addEventListener("click", async () => {
  const devices = await navigator.mediaDevices.enumerateDevices();
  const container = document.querySelector<HTMLDivElement>("#device-buttons-container");
  ...
  document.querySelector<HTMLDivElement>("#start-video-devices-buttons")!.innerHTML = videoButtonsHtml;

  document.querySelectorAll<HTMLButtonElement>(".start-video-device-button").forEach((element) => {
  ...
  document.querySelector<HTMLDivElement>("#start-audio-devices-buttons")!.innerHTML = audioButtonsHtml;

  document.querySelectorAll<HTMLButtonElement>(".start-audio-device-button").forEach((element) => {
    const deviceId = element.dataset.deviceId;
    if (!deviceId) throw Error("Device id is null");
...
export const startLastSelectedDevice = () => {
  const videoDeviceId: string | null = localStorage.getItem(LAST_SELECTED_VIDEO_DEVICE_ID_KEY);
...
export const createVideoComopnent = (trackId: string, stream: MediaStream): HTMLVideoElement => {
